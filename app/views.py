...
def placeorder(req):
    if req.method == 'POST':
        ...
        if product_id:
            ...
            req.session['singleorderamount'] = total
            cartdata.append({
                'productname':cartitem.productid.productname,
                'qty':cartitem.qty,
                'price':cartitem.productid.price,
            ...
            req.session['allorderamount']=total
        ...
        try:
            ...
            try:
                ...
                if req.session.get("product_id"):
                    ...
                    allorderamount = req.session.get("allorderamount")
                    for item in cart_items:
                        Order.objects.create(
                            user=user,
                ...
                req.session['amountpaid'] = client.order.fetch(order_id)["amount"]
...
def payment_success(req):
    ...
    amount = req.session.get('amountpaid')
    user = req.user
...
def showorders(req):
    user = req.user
    orders = Order.objects.filter(user=user).select_related('producttid','address').order_by('-orderdate')
    for order in orders:
        order.orderamount=order.qty*order.producttid.price
        if not order:
